import { NavbarLanding } from "@/components/landing/NavbarLanding";
import { FooterSection } from "@/components/landing/FooterSection";
import { Monitor, Apple, Smartphone, Download as DownloadIcon, CheckCircle, Shield } from "lucide-react";
import { Button } from "@/components/ui/button";

const platforms = [
  {
    icon: Monitor,
    name: "Windows",
    version: "v2.4.0",
    size: "~85 MB",
    requirements: "Windows 10 ou superior",
    format: ".exe",
    available: true,
    downloadUrl: "#",
  },
  {
    icon: Apple,
    name: "macOS",
    version: "v2.4.0",
    size: "~90 MB",
    requirements: "macOS 11 Big Sur ou superior",
    format: ".dmg",
    available: true,
    downloadUrl: "#",
  },
  {
    icon: Smartphone,
    name: "Android",
    version: "v2.4.0",
    size: "~45 MB",
    requirements: "Android 10 ou superior",
    format: ".apk",
    available: false,
    downloadUrl: "#",
  },
  {
    icon: Smartphone,
    name: "iOS",
    version: "v2.4.0",
    size: "~50 MB",
    requirements: "iOS 15 ou superior",
    format: "App Store",
    available: false,
    downloadUrl: "#",
  },
];

const desktopFeatures = [
  "Acesso offline aos dados da clínica",
  "Notificações nativas do sistema",
  "Inicialização automática",
  "Performance otimizada",
  "Atualizações automáticas",
  "Atalhos de teclado nativos",
];

export default function Download() {
  return (
    <div className="min-h-screen bg-background">
      <NavbarLanding />

      {/* Hero */}
      <section className="pt-32 pb-16 px-6 bg-gradient-to-b from-primary/5 to-background">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-sm font-medium mb-6">
            <DownloadIcon className="w-4 h-4" />
            Disponível para Desktop
          </div>
          <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">Baixe o DermaCore</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Instale o DermaCore no seu computador ou celular para ter acesso rápido e notificações nativas.
          </p>
        </div>
      </section>

      {/* Download Cards */}
      <section className="py-16 px-6">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {platforms.map((p, i) => (
            <div key={i} className={`p-6 rounded-xl border bg-card flex flex-col ${
              p.available ? "border-border hover:border-primary/50" : "border-border/50 opacity-70"
            } transition-colors`}>
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                  p.available ? "bg-primary/10" : "bg-muted"
                }`}>
                  <p.icon className={`w-6 h-6 ${p.available ? "text-primary" : "text-muted-foreground"}`} />
                </div>
                <div>
                  <h3 className="font-semibold text-foreground">{p.name}</h3>
                  <span className="text-xs text-muted-foreground">{p.version}</span>
                </div>
              </div>

              <div className="space-y-2 text-sm text-muted-foreground mb-6 flex-1">
                <p>Tamanho: {p.size}</p>
                <p>Formato: {p.format}</p>
                <p>{p.requirements}</p>
              </div>

              {p.available ? (
                <Button className="w-full btn-premium text-white" asChild>
                  <a href={p.downloadUrl}>
                    <DownloadIcon className="w-4 h-4 mr-2" />
                    Baixar para {p.name}
                  </a>
                </Button>
              ) : (
                <Button className="w-full" variant="outline" disabled>
                  Em breve
                </Button>
              )}
            </div>
          ))}
        </div>
      </section>

      {/* Features */}
      <section className="py-16 px-6 bg-muted/30">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold text-center mb-8">Vantagens do App Desktop</h2>
          <div className="grid md:grid-cols-2 gap-4">
            {desktopFeatures.map((f, i) => (
              <div key={i} className="flex items-center gap-3 p-4 rounded-lg bg-card border border-border">
                <CheckCircle className="w-5 h-5 text-emerald-500 flex-shrink-0" />
                <span className="text-foreground">{f}</span>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Security note */}
      <section className="py-16 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <Shield className="w-10 h-10 text-primary mx-auto mb-4" />
          <h2 className="text-xl font-bold mb-2">Download Seguro</h2>
          <p className="text-muted-foreground max-w-lg mx-auto">
            Todos os downloads são assinados digitalmente e verificados. 
            Seus dados são protegidos com criptografia ponta a ponta.
          </p>
        </div>
      </section>

      {/* Web alternative */}
      <section className="py-12 px-6 bg-gradient-to-r from-primary/5 to-accent/5">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-xl font-bold mb-2">Prefere usar no navegador?</h2>
          <p className="text-muted-foreground mb-4">O DermaCore funciona perfeitamente em qualquer navegador moderno.</p>
          <Button variant="outline" asChild>
            <a href="/auth">Acessar pelo Navegador</a>
          </Button>
        </div>
      </section>

      <FooterSection />
    </div>
  );
}
